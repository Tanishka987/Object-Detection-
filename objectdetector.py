...
Known_widths = {'person': 16,'car': 72,'bicycle': 20,'bed': 60,'couch': 80,'chair': 20,'mirror': 24,'dining table': 60,
    'window': 48,'desk': 48,'toilet': 16,'door': 36,'tv': 45,'laptop': 13,'mouse': 2.5,'remote': 2,'book': 6,'scissors': 3,
    'teddy bear': 10,'cell phone ':2.5,}
...
while True:
    ...
    if len(classIds) != 0:
        for i in indices:
            ...
            x, y, w, h = box[0], box[1], box[2], box[3]
            cv2.rectangle(frame, (x, y), (x + w, y + h), color, thickness=2)
            cv2.putText(frame, label + " " + confidence, (x + 10, y + 20), font, 1, color, 2)
    ...
    if len(bbox) == 0:
        ...
        frame = cv2.putText(frame, str(D / 10) + " cm", (300, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2,
                            cv2.LINE_AA)
